...
export function StageSendPrompt({ prompt, lang = 'he', onClose, onSent }) {
  ...
  return (
    <div onMouseDown={e => e.target === e.currentTarget && onClose()} style={{ position: 'fixed', inset: 0, zIndex: 2600, background: 'rgba(0,0,0,.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 14 }}>
      <div dir={lang === 'en' ? 'ltr' : 'rtl'} style={{ width: '100%', maxWidth: 480, background: '#10121E', border: '1px solid rgba(37,211,102,.35)', borderRadius: 16, padding: 18, color: '#E8E4D8', boxShadow: '0 24px 70px rgba(0,0,0,.6)', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ fontSize: 16, fontWeight: 800 }}>💬 {t.title}</div>
        <div style={{ fontSize: 13, color: 'rgba(232,228,216,.7)' }}><b style={{ color: '#E8E4D8' }}>{prompt.lead.name || prompt.lead.phone}</b> · {t.moved} "{prompt.stageName || prompt.stage}"</div>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
          <button onClick={() => act('skip')} disabled={!!busy} style={b('transparent', 'rgba(232,228,216,.2)', 'rgba(232,228,216,.65)')}>{t.skip}</button>
