...
// Names of feeds that are dedicated real-estate sections (relaxes the classifier's corroboration rule)
const TRUSTED_NAMES = new Set(RSS_SOURCES.filter(s => s.trusted).map(s => s.name))
export function isTrustedSource(source) {
  const n = String(source || '')
  return TRUSTED_NAMES.has(n) || /נדל"?ן|נדלן|real ?estate/i.test(n)
}

// og:image / twitter:image of an article page (bounded, never throws)
export async function fetchOGImage(url, timeoutMs = 3000) {
  try {
    const r = await fetch(url, { headers: { ...RSS_HEADERS, Accept: 'text/html,application/xhtml+xml,*/*' }, signal: AbortSignal.timeout(timeoutMs), redirect: 'follow' })
    if (!r.ok) return ''
    try { if (new URL(r.url).hostname.includes('google.')) return '' } catch {}
...
// Run fn over items with bounded concurrency and an overall time budget; unfinished items get `undefined`.
export async function mapWithBudget(items, fn, { concurrency = 6, budgetMs = 20000 } = {}) {
