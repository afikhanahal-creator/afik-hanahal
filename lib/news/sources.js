...
const NAME_KEY = [
  ...
  [/walla|וואלה/i, 'walla'], [/israel ?hayom|ישראל היום/i, 'israelhayom'], [/כאן|kan/i, 'kan'],
  [/bizportal|ביזפורטל/i, 'bizportal'], [/ice/i, 'ice'], [/yad2|יד2|יד 2/i, 'yad2'], [/דבר|davar/i, 'davar1'],
...
// Fetch every source with bounded concurrency (Google News rate-limits bursts).
export async function fetchAllSources({ timeoutMs = 8000, concurrency = 10, log = () => {} } = {}) {
  ...
  async function worker() {
    while (queue.length) {
      const s = queue.shift()
      try {
        const r = await fetch(s.url, { headers: RSS_HEADERS, signal: AbortSignal.timeout(timeoutMs), redirect: 'follow' })
        if (!r.ok) { log(`${s.name}: HTTP ${r.status}`); continue }
        const items = parseRSS(await r.text(), s.name, { gn: !!s.gn, trusted: !!s.trusted })
        log(`${s.name}: ${items.length} items`)
        out.push(...items)
...
export function cleanTitle(title, gn) {
  ...
  return cut > 10 ? t.slice(0, cut).trim() : t
}
