...
const CLEAR = compile([
  ...
  'מקרקעין',
  bare('לשכת ?רישום'), bare('נסח ?טאבו'), bare('רישום ?בטאבו'), bare('רשומ(?:ה|ים|ות)? ?בטאבו'), bare('העברה ?בטאבו'), bare('טאבו ?(?:על|של) ?ה?(?:דירה|נכס|קרקע)'),
  bare('פינוי[ -]?בינוי'),
  ...
  bare('מס ?ה?רכישה'), bare('מס ?ה?שבח'), bare('היטל ?ה?השבחה'), bare('היטלי ?השבחה'), bare('מס ?דירה ?שלישית'), bare('ריבוי ?דירות'),
  ...
  bare('דיור ?(?:ציבורי|בר ?השגה|להשכרה)'), bare('משפרי ?דיור'), bare('יוקר ?הדיור'), bare('מצוקת ?ה?דיור'), bare('מחירי ?הדיור'), bare('פתרונות ?דיור'),
  bare('בינוי ?ו?ה?שיכון'), bare('משרד ?השיכון'),
  bare('צמוד(?:ת|י|ות)? ?קרקע'), bare('בית ?פרטי'), bare('בתים ?פרטיים'), bare('בתי ?מגורים'), bare('דו ?משפחתי'),
...
const AMBIG = compile([
  ...
  'מגורים', 'למגורים',
  'דיור', 'טאבו',
  'פנטהאוז', 'פנטהאוס', 'דופלקס', 'קוטג', "קוטג'", 'וילה', 'וילות',
])
...
const VETO = compile([
  ...
  // home tours / renovation (décor magazines) and infrastructure (engineering feeds)
  'שיפוץ', 'שיפצו', 'שיפצנו', 'סיור', 'ריצוף', 'פרקט', 'תאורה', 'וילונות', 'גינה', 'גינון', 'צמחים', 'חצר', 'פינת', 'ספה', 'שטיח',
  'כביש', 'כבישים', 'מנהרה', 'מנהרות', 'גשר', 'גשרים', 'רכבת', 'מסילה', 'מחלף', 'נתיבי', 'נמל', bare('שדה ?ה?תעופה'), bare('תחנת ?כוח'), 'מתקן',
  // macro-economy / markets (economy feeds)
  'בורסה', 'מניה', 'מניות', 'המניה', bare('אג"ח'), 'דולר', 'אינפלציה', 'פנסיה', 'פנסיוני', bare('ביטוח ?לאומי'), bare('מס ?הכנסה'), bare('שכר ?ה?מינימום'), 'תקציב', 'משכורת', 'משכורות',
  bare('דיור ?מוגן'), bare('שובר(?:ת|ים)? ?(?:כל )?טאבו'),
  // health
