...
// injected automatically). Used when Supabase refuses a write (quota, paused project, outage) so a
...
export async function backupPut(pathname, obj) {
  ...
  const r = await fetch(`${API}/${pathname}`, { method: 'PUT', headers: H({ 'x-content-type': 'application/json', 'x-add-random-suffix': '1', 'x-cache-control-max-age': '0' }), body: JSON.stringify(obj), signal: AbortSignal.timeout(15000) })
