...
// Ask every source at once. The answer of the highest-priority source that succeeds wins, and we
// return as soon as it's known (every source above it has already failed), not when the slowest ends.
export function resolveParcel(gush, helka, { fetchImpl = fetch, timeoutMs = 7000 } = {}) {
  ...
  const state = list.map(() => null)   // null = pending · { ok, value | error }
  return new Promise(done => {
    const settle = () => {
      const tried = () => state.map((s, i) => s && { source: list[i].name, ok: s.ok, ...(s.ok ? {} : { error: s.error }) }).filter(Boolean)
      for (let i = 0; i < state.length; i++) {
        if (!state[i]) return                                   // a better source may still answer
        if (state[i].ok) return done({ ok: true, gush: g, helka: h, ...state[i].value, tried: tried() })
      }
      done({ ok: false, gush: g, helka: h, error: 'not_found', tried: tried() })
    }
    list.forEach((s, i) => {
      trySource(s, fetchImpl, timeoutMs)
        .then(value => { state[i] = { ok: true, value } }, e => { state[i] = { ok: false, error: String(e?.message || e).slice(0, 160) } })
        .then(settle)
    })
  })
}
