...
const cdnImg = (url, w) => (url && /^https?:\/\//.test(url) ? `https://wsrv.nl/?url=${encodeURIComponent(url)}&w=${w}&q=72&output=webp&we` : url)
...
#afik-pre .ap-img{width:100%;aspect-ratio:16/10;object-fit:cover;display:block;background:#15162A}
...
@media (min-width:900px){#afik-pre .ap-wrap{padding-top:28px}#afik-pre .ap-img{border-radius:18px}}
...
export function renderLanding(template, p, { origin, lang = 'he' } = {}) {
  ...
  const head = `
    ...
    ${imgSm ? `<link rel="preload" as="image" href="${esc(imgSm)}" fetchpriority="high" />` : ''}
    <style>${PRE_CSS}</style>
    <script>
    (function(){try{
  ...
  const body = `
    <div id="afik-pre" dir="${lang === 'en' ? 'ltr' : 'rtl'}" aria-busy="true">
      <main class="ap-wrap">
        ${imgSm ? `<img class="ap-img" src="${esc(imgSm)}" alt="${esc(m.title)}" fetchpriority="high" decoding="async" onerror="if(!this.dataset.f){this.dataset.f=1;this.src=${esc(JSON.stringify(img))}}else{this.style.display='none'}" />` : ''}
        <div class="ap-body">
  ...
  html = html.replace(/(<meta charset="[^"]*"\s*\/?>)/i, `$1${head}`)
